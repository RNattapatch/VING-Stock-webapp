...
### Step 1 — กรอก API URL
> **Web App URL ฝังในโค้ดให้แล้ว** (ตัวแปร `HARDCODED_API` ในไฟล์ html) — ทุกเครื่องเปิดเว็บมาใช้ได้เลย ไม่ต้องกรอก
> ⚙ ใช้เฉพาะตอนอยาก override URL ชั่วคราว (เช่น ทดสอบ deployment ใหม่)
> ถ้า re-deploy Apps Script ได้ URL ใหม่ → แก้ `HARDCODED_API` ใน 3 ไฟล์ (index/checker/stock) แล้ว push GitHub ใหม่
...
1. login → กด **นับสต็อก**
2. ยิงได้ 2 ทาง:
   - **เครื่องสแกน USB / พิมพ์เอง** (บน PC) — โฟกัสอยู่ที่ช่องยิงเสมอ พิมพ์ SKU + Enter
   - **กล้องมือถือ** — กดปุ่ม **📷** ข้างช่องยิง → เล็ง barcode ในกรอบ → ยิงต่อเนื่องได้ (มีเสียง beep ยืนยัน)
3. ยิงซ้ำ SKU เดิม = **+1 ชิ้นทันที** (กล้อง: ยิงซ้ำได้ทุก 1.5 วินาที กันนับรัวจากเฟรมเดียว)
